...
| Treat prose-only edits (Objects/Actions wording) as low-value churn. | Flag **prose↔root-reference drift**: the install/verify shell blocks in `SEED.md`'s `## Dependencies` / `## Verify` diverging from the root reference files (`api/`, `src/`, `server.js`) they drive — the canonical SEED regression. |
| Suggest "approve all" / batched shell to speed an install script. | Flag any install/verify shell block in `SEED.md` (`## Dependencies` / `## Verify`) that **batches or auto-approves** — violates `tier-2` per-block confirm (`^act-trust`). |
