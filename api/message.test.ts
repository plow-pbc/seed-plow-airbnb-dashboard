...
const TOKEN = 'secret';
const URL = 'https://x/api/message';

function getMessage(store: MessageStore, bearer = TOKEN) {
  return createMessageHandler({ store, token: TOKEN })(
    new Request(URL, { headers: { Authorization: `Bearer ${bearer}` } }),
  );
}

function postMessage(store: MessageStore, body: unknown, bearer = TOKEN) {
  return createMessageHandler({ store, token: TOKEN })(
    new Request(URL, {
      method: 'POST',
...
describe('createMessageHandler', () => {
  it('GET returns null when no message is set', async () => {
    const res = await getMessage(fakeStore());
  ...
  it('GET returns the stored message', async () => {
    ...
    const res = await getMessage(fakeStore(msg));
    expect(await res.json()).toEqual({ message: msg });
  });

  it('rejects requests with the wrong bearer token', async () => {
    const res = await getMessage(fakeStore(), 'nope');
    expect(res.status).toBe(401);
  });

  it('rejects requests with no Authorization header', async () => {
    const res = await createMessageHandler({ store: fakeStore(), token: TOKEN })(
      new Request(URL),
    );
    expect(res.status).toBe(401);
  });
  ...
  it('POST writes a message and returns it', async () => {
    ...
    const res = await postMessage(store, {
      text: 'You are loved.',
  ...
  it('POST rejects a non-ISO expires_at', async () => {
    const res = await postMessage(fakeStore(), { text: 'ok', expires_at: 'tomorrow' });
    expect(res.status).toBe(400);
  });

  it('POST rejects expires_at without a timezone offset', async () => {
    const res = await postMessage(fakeStore(), {
      text: 'ok',
